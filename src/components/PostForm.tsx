...
interface PostFormProps { refreshPosts: () => void; user: User; }
interface PostSuccessResponse { message: string; txHash?: string; }
...
export default function PostForm({ refreshPosts, user }: PostFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!content.trim()) { toast.error('分享内容不能为空！'); return; }
    const token = localStorage.getItem('jwt_token');
    if (!token) { toast.error('登录状态已失效，请重新登录。'); return; }
    ...
    try {
      const response = await fetch('/api/posts', {
        ...
        body: JSON.stringify({ type: isMintMode ? 'web3' : 'web2', content: content }),
      });
      if (!response.ok) {
      ...
      const successMessage = isMintMode ? `存证成功！` : '帖子发布成功！';
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800 border border-gray-700 p-6 rounded-lg shadow-lg mb-10">
      <h2 className="text-xl font-semibold mb-4 text-white">欢迎, <span className="font-mono text-cyan-400">{user.phoneNumber}</span>!</h2>
      <div>
        <textarea
          value={content}
      ...
      <div className="my-4">
        <label className="flex items-center text-gray-400 cursor-pointer select-none">
          <input type="checkbox" checked={isMintMode} onChange={(e) => setIsMintMode(e.target.checked)} className="w-4 h-4 text-cyan-600 bg-gray-700 border-gray-600 rounded focus:ring-cyan-500" />
